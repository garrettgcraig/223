...
map1 <- tm_shape(zion_elevation) +
  ...
  tm_layout(main.title = "Zion National Park Elevation",
            main.title.position = "center",
            legend.outside = TRUE)
map1

map2 <- tm_shape(zion_land) +
  tm_raster(palette = "Set3", title = "Land Cover") +
  tm_layout(main.title = "Zion National Park Land Cover",
            main.title.position = "center",
            legend.outside = TRUE)
map2

#combine the two plots into one; make sure the sizes of the windows are comparabale
...
#find out the number oof layers in raster
nlyr(landsat)
```
...
```{r}
landsat3 <- subset(landsat,3)  # by layer number
landsat4 <- subset(landsat,"landsat_4")  # by layer name
```
...
#merge austria ad switzerland rasters
...
map1 <- tm_shape(austria) +
  tm_raster(palette = terrain.colors(10), title = "Elevation (m)") +
  tm_layout(main.title = "Austria Elevation",
            main.title.position = "center",
            legend.outside = TRUE)

map2 <- tm_shape(switzerland) +
  tm_raster(palette = terrain.colors(10), title = "Elevation (m)")
  tm_layout(main.title = "Switzerland Elevation",
            main.title.position = "center",
            legend.outside = TRUE)
  
map3 <- tm_shape(merged) +
  tm_raster(palette = terrain.colors(10), title = "Elevation (m)") +
  tm_layout(main.title = "Austria + Switzerland Elevation",
            main.title.position = "center",
            legend.outside = TRUE)
...
# get cell ID for row 10, column 15
cell_id <- cellFromRowCol(zion_elevation, 10, 15)
...
```{r}
#create point within area coveredby raster
point <- matrix(c(-113,37.5),ncol=2)

#approach 1
id = terra::cellFromXY(zion_elevation, point)
#index the cell
zion_elevation[id]

#approach2

#extractrastervalueatpoint
...
```{r}
#find the extent ofcurrent raster
ext(zion_elevation)
...
#verifythat output has fewer cellsthan original
if (ncell(zion_elevation_clip) < ncell(zion_elevation)) {
  print("Clipping worked, fewer cells in clipped version")
} else {
  print("Clipping did not work, same number of cells")
}
```
