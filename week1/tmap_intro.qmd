...
tm_shape(world_vector) + #defines the spatial object to plot
  tm_polygons() #defines how to plot the spatial object
```
...
```{r}
tm_shape(world_vector) + # first spatial object
  tm_polygons() + # how to plot the first spatial object
  tm_shape(world_cities) + # second spatial object
  tm_dots()  # how to plot the second spatial object
...
```{r}
tm_shape(world_elevation) + # first spatial object
  tm_raster() + # how to plot the first spatial object
  tm_shape(world_vector) + # second spatial object
  tm_borders() + # how to plot the second spatial object
  tm_shape(world_cities) + # third spatial object
  tm_dots() + # how to plot the third spatial object
  tm_text("name", size=0.7,col = "darkblue") + # add city names 
# add white box around city names 
  tm_layout(frame = FALSE) # remove frame around map
```
...
```{r}
tm_shape(world_cities) + # first spatial object
  tm_dots() + # how to plot the first spatial object
  tm_text("name", size=0.7,col = "darkblue") + # add city names
  tm_shape(world_elevation) + # second spatial object
  tm_raster() + # how to plot the second spatial object
  tm_shape(world_vector) + # third spatial object
  tm_borders()  # how to plot the third spatial object
```
...
tm_shape(world_elevation) + # first spatial object
  ...
  tm_shape(world_vector) + # second spatial object
  tm_borders() + # how to plot the second spatial object
  tm_shape(world_cities,is.main = TRUE) + # third spatial object 
  tm_dots() + # how to plot the third spatial object
  tm_text("name", size=0.7,col = "darkblue") # add city names
...
```{r}
tm_shape(world_elevation,bbox=c(-15,50,-65,-14)) + # first spatial object
  tm_raster() + # how to plot the first spatial object
  tm_shape(world_vector) + # second spatial object
  tm_borders()  # how to plot the second spatial object
...
```{r}
tm_shape(world_elevation,bbox="Africa") + # first spatial object
  tm_raster() + # how to plot the first spatial object
  tm_shape(world_vector) + # second spatial object
  tm_borders() + # how to plot the second spatial object
  tm_shape(world_cities) + # third spatial object
  tm_dots() + # how to plot the third spatial object
  tm_text("name", size=0.7,col = "darkblue") + # add city names
  tm_add_legend(frame = FALSE) # remove frame around map
```
...
tm_shape(world_elevation,bbox="Kenya") + # first spatial object
  tm_raster() + # how to plot the first spatial object
  tm_shape(world_vector) + # second spatial object
  tm_borders() + # how to plot the second spatial object
  tm_shape(world_cities) + # third spatial object
...
```{r}
tm_shape(world_vector) +
  tm_polygons(col = "lightblue", border.col = "black", lwd=0.2,lty="solid")+
  tm_layout(frame = FALSE) # remove frame around map
...
```{r}
tm_shape(world_vector) +
  tm_borders(col = "red", lwd=0.2,lty="solid")
```
...
```{r}
#fill polygon with color palette representing variable
tm_shape(world_vector)+
  tm_fill("hdi", style = "quantile", n=5, palette = "Blues", title="HDI")+
  tm_layout(frame = FALSE) # remove frame around map
```
...
tm_shape(world_vector) +
  tm_polygons(
    ...
    fill.legend = tm_legend(title = "Regional Income (USD)",
                            position = tm_pos_out("right")),
  ) +
  tm_title("Global Income", frame = FALSE )  +
  tm_layout(
    style = "natural_v3",
    bg.color = "lightblue",
...
tm_shape(world_vector)+
  tm_polygons(fill="life_expectancy",
              fill.scale=tm_scale(values="parks.torres"),
              fill.legend=tm_legend(title="Life Expectancy (years)",
                                    position=tm_pos_out ("left")))

## run cols4all::c4a_palettes() to view all color palettes
```
...
tm_shape(world_vector)+
  ...
  tm_symbols(
    size="pop2020",
    size.legend = tm_legend(title="Population 2020",
                            position=tm_pos_out ("right"),
...
tm_shape(world_vector) +
  tm_polygons(
    col = "wb_income_region", 
    title = "Regional Income"
  ) +
  tm_layout(
    title = "World Bank Income Regions",
    title.frame = TRUE,  # adds a frame around the title
    style = "watercolor",
    bg.color = "grey95"
  )
```
...
tm_shape(world_vector) +
  tm_polygons(fill="gdp_per_cap",
              fill.legend=tm_legend(title="GDP per capita (USD)",            position=tm_pos_out ("right"))) 
```
...
tmap_save(
  tm = map1,
  filename = here("week1", "outputs", "map1.png"),
  width = 10,   # inches
  height = 6,   # inches,
  dpi = 300
)
