...
```{r}
london_point <- st_sfc(st_point(c(-0.12755, 51.507222)))
```
...
```{r}
#combine geometry and attributes into a sf object
...
```{r}
world_df <- world %>%
  select(-geom) # this DOESN'T actually do anything
```
...
```{r include=TRUE}
world_df <- world %>%
  st_drop_geometry() # this DOES drop the geometry column
```
...
```{r}
london_proj <- data.frame(x=530000, yy = 180000) |> 
  st_as_sf(coords = c("x", "yy"), crs = "EPSG:27700")
```
...
```{r}
if(st_crs(london_proj)==st_crs(london_sf_transformed)){print("CRS match!")} else {print("CRS do not match!")}
```
...
tm_shape(world_coffee) +
  tm_polygons("coffee_production_2017", title = "Coffee production (2017)", 
              style = "quantile", n = 5, 
              palette = "Greens", 
              alpha = 0.7, 
              textNA = "No data", colorNA = "lightgrey") +
  tm_layout(title = "Coffee production by country",
            legend.outside = TRUE)

##REPEAT ABOVE BUT MAKE SURE TITLE IS OUTSIDE OF MAP
tm_shape(world_coffee) +
  tm_polygons("coffee_production_2017", title = "Coffee production (2017)", 
              style = "quantile", n = 5, 
              palette = "copper", 
              alpha = 0.7, 
              textNA = "No data", colorNA = "lightgrey") +
  tm_layout(main.title = "Coffee production by country",
            legend.outside = TRUE)  
```
...
if(nrow(world_coffee_inner)<nrow(coffee_data)){
  warning("Some countries in coffee_data did not match with world data.")
}
```
